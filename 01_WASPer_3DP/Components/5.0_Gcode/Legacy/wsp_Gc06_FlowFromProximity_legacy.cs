// -----------------------------------------------------------------------------
// wsp_Gc06_Flow from Proximity  — v6  (hybrid)
// -----------------------------------------------------------------------------
// ARCHITECTURE
// -----------------------------------------------------------------------------
//
...
{
    public class wsp_Gc06_FlowFromProximity_v6_Final_Legacy : GH_Component
    {
        private readonly string _versionTag;

        public wsp_Gc06_FlowFromProximity_v6_Final_Legacy()
            : base(
                "wsp_Gc06_Flow from Proximity Legacy",
                "ProxFlow",
        ...
        public override Guid ComponentGuid =>
            new Guid("A3C17E89-52FD-4B0A-9D3E-1F8B7C6A2D04");
        ...
        protected override void RegisterInputParams(GH_InputParamManager p)
        {
            ...
            p.AddPointParameter(
                "p_points", "p_points",
                "Printing points as DataTree {layer;curve}.",
                GH_ParamAccess.tree);
            p[0].Optional = true;
            ...
            p.AddPlaneParameter(
                ...
                "Optional reference planes, one per layer. World XY used when absent.",
                GH_ParamAccess.list);
            p[1].Optional = true;
            ...
            p.AddNumberParameter(
                "min_flux", "min_flux",
                "Flow value in maximally crowded regions.",
                GH_ParamAccess.item, 0.60);

            // 3
            p.AddNumberParameter(
                "max_flux", "max_flux",
                "Flow value in safe open regions.",
                GH_ParamAccess.item, 1.00);

            // 4
            p.AddNumberParameter(
                "d_crit", "d_crit",
                "Clearance at or below which risk = 1  (min_flux).",
                GH_ParamAccess.item, 2.0);

            // 5
            p.AddNumberParameter(
                "d_safe", "d_safe",
                "Clearance at or above which risk = 0  (max_flux).",
            ...
            // 7
            p.AddNumberParameter(
                "curv_threshold", "curv_th",
                "Absolute turning-angle threshold in RADIANS for bend detection.\n\n" +
                "A contiguous region of vertices whose turning angle >= this value\n" +
                "is a turning event.  NMS picks the single sharpest vertex per event\n" +
                "as the bend peak — one peak per geometric bend, always.\n\n" +
                "0.3 rad (~17°)  good default for sinusoidal toolpaths\n" +
                "0.5 rad (~29°)  only sharp bends\n" +
                "0.15 rad (~9°)  also catches gentle curves\n\n" +
                "Visualise cluster_id to validate: each colour change = one zone boundary.",
            ...
            p.AddIntegerParameter(
                ...
                "Number of points on each side of a detected bend peak to mark as\n" +
                "turning-zone geometry.  These points and their segments are EXCLUDED\n" +
                "from self-clearance — they act as separators between runs.\n\n" +
                "2  — excludes 5 points per bend  (default)\n" +
                "4  — excludes 9 points per bend  (use for dense toolpaths)\n" +
                "0  — only the peak point itself is excluded",
                GH_ParamAccess.item, 2);
            p[8].Optional = true;

            p.AddGenericParameter(
                "wasper_path", "wsp_path",
                "Optional WASPer Print Path object. Explicit legacy inputs override its fields; the object supplies p_points when the legacy point tree is not connected.",
                GH_ParamAccess.item);
            p[9].Optional = true;
        }
        ...
        {
            p.AddNumberParameter("flows", "flows",
                "Per-point flow value.", GH_ParamAccess.tree);
            p.AddNumberParameter("clearance", "clearance",
                "Per-point combined clearance (-1 if no valid neighbour found).", GH_ParamAccess.tree);
            p.AddNumberParameter("risk", "risk",
                "Per-point crowding risk [0,1].", GH_ParamAccess.tree);
            p.AddNumberParameter("self_clearance", "self_clr",
                "Per-point min distance to segments in other runs, after smoothing.", GH_ParamAccess.tree);
            p.AddNumberParameter("other_clearance", "other_clr",
                "Per-point min distance to other curves, after smoothing.", GH_ParamAccess.tree);
            p.AddIntegerParameter("cluster_id", "cluster_id",
                "Per-point run index.  -1 = turning-zone point (excluded separator).\n" +
                "Colour changes mark zone boundaries.  Validate curv_threshold here.", GH_ParamAccess.tree);
            p.AddGenericParameter(
                "wasper_path", "wsp_path",
                "WASPer Print Path object carrying the processed points, inherited path metadata, and calculated flows.",
                GH_ParamAccess.item);
        }
        #endregion
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            bool hasPackedPath = global::WASPer_3DP.WasperGcodeTreeUtil.TryGetPrintPath(DA, 9, out packedPath);
            ...
            var inputPlanes = new List<Plane>();
            DA.GetDataList(1, inputPlanes);
            if (inputPlanes.Count == 0 && hasPackedPath && packedPath.HasPlanes)
                inputPlanes = ExtractLayerPlanes(packedPath.PtPlanes);
            ...
            if (!DA.GetData(2, ref minFlux)) return;
            if (!DA.GetData(3, ref maxFlux)) return;
            if (!DA.GetData(4, ref dCrit)) return;
            if (!DA.GetData(5, ref dSafe)) return;
            if (!DA.GetData(6, ref clusterMode)) return;
            DA.GetData(7, ref curvThr);
            DA.GetData(8, ref zoneHalfW);

            Message = _versionTag;

            if (pTree == null || pTree.PathCount == 0)
            { AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "No points in p_points."); return; }
            ...
            foreach (var br in branches)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    double selfOut = br.SelfClearance[i] >= Huge * 0.5 ? -1.0 : br.SelfClearance[i];
                    double otherOut = br.OtherClearance[i] >= Huge * 0.5 ? -1.0 : br.OtherClearance[i];
                    double clrOut = br.Clearance[i] >= Huge * 0.5 ? -1.0 : br.Clearance[i];

                    fluxTree.Append(new GH_Number(br.Flux[i]), path);
                    clearTree.Append(new GH_Number(clrOut), path);
                    riskTree.Append(new GH_Number(br.Risk[i]), path);
                    selfClrTree.Append(new GH_Number(selfOut), path);
                    otherClrTree.Append(new GH_Number(otherOut), path);
                    ridTree.Append(new GH_Integer(br.RunId[i]), path);
                }
            }

            DA.SetDataTree(0, fluxTree);
            DA.SetDataTree(1, clearTree);
            DA.SetDataTree(2, riskTree);
            DA.SetDataTree(3, selfClrTree);
            DA.SetDataTree(4, otherClrTree);
            DA.SetDataTree(5, ridTree);

...
/*
Component: wsp_Gc04_Marlin Gcode (From points)
Nickname: Marlin Gcode
Category: WASPer_3DP
SubCategory: 5.0_Gcode
Version:
    Uses the compiled assembly version in the component message via _versionTag.
    LDM path: C# port of GhPython wsp_Gc04 LDM v1.0.4 (260310).
    FDM path: C# port of GhPython wsp_Gc04 FDM v1.0.4 (260207).
...
3dp_params object selects the code path:
  ...
  - wsp_Gc02_LDM 3DP Parameters (Process = LDM):
  ...
  - wsp_Gc03_FDM 3DP Parameters (Process = FDM):
...
1) wsp_path    : WasperPrintPath       Packed p_points / pt_planes / flows /
                                        layer_h from Gc01; sole path source.
...
6) layers          int           Total number of layers.
7) wsp_path        WasperPrintPath
...
{
    public sealed class wsp_Gc04_Marlin_Gcode_From_Points_Legacy : GH_Component
    {
        private readonly string _versionTag;

        public wsp_Gc04_Marlin_Gcode_From_Points_Legacy()
            : base(
                "wsp_Gc04_Marlin Gcode (From WASPer Path)",
                "Marlin Gcode",
                "Generates Marlin-compatible G-code from a DataTree of points {layer;curve}. " +
                "The wired 3dp_params object selects the process: LDM (wsp_Gc02_LDM 3DP " +
                "Parameters — splitting, time correction) or FDM (wsp_Gc03_FDM 3DP Parameters " +
                "— fan, temperatures, custom start/end blocks).\r\n" +
                "Path data arrives via wsp_path (packed, from Gc01) or the individual trees; " +
                "wired trees always win. Unwired 3dp_params runs with LDM defaults. " +
        ...
        public override Guid ComponentGuid =>
            new Guid("5C7E9A14-6B2D-4F08-A3E1-97D45B8C2E66");
        ...
        protected override void RegisterInputParams(GH_InputParamManager p)
        {
            ...
            i = p.AddGenericParameter("wasper_path", "wsp_path",
                "WASPer Print Path object from Gc01 packing p_points, pt_planes, flows " +
                "layer_h, layer_w, layer_wf, print_vol, nozzle_diam, and optional Gc08 " +
            ...
            i = p.AddGenericParameter("3dp_params", "3dp_params",
                ...
                "printing parameters: wsp_Gc02_LDM 3DP Parameters (Process = LDM) or " +
                "wsp_Gc03_FDM 3DP Parameters (Process = FDM). When not wired, the " +
        ...
        {
            p.AddTextParameter("g_code", "g_code",
                "Generated G-code lines. LDM: one branch per output file ({0} when " +
                "not split). FDM: single branch {0}.",
                GH_ParamAccess.tree);

            p.AddPointParameter("printing_points", "printing_points",
                "Pass-through of the printing points read from wsp_path and used by the generator.",
                GH_ParamAccess.tree);

            p.AddNumberParameter("printing_speed", "printing_speed",
                "Resolved per-point print feedrate in mm/min, matching printing_points.",
                GH_ParamAccess.tree);

            p.AddNumberParameter("p_time_min", "p_time_min",
                "Total estimated print time in minutes (LDM: time_correction applied).",
                GH_ParamAccess.item);

            p.AddNumberParameter("p_path_len_mm", "p_path_len_mm",
                "Total printed path length in mm.",
                GH_ParamAccess.item);

            p.AddNumberParameter("p_volume_cm3", "p_volume_cm3",
                "Total extruded volume in cm3.",
                GH_ParamAccess.item);

            p.AddIntegerParameter("layers", "layers",
                "Total number of layers.",
                GH_ParamAccess.item);

            p.AddGenericParameter("wasper_path", "wsp_path",
                "WASPer Print Path object carrying the generated points and resolved metadata " +
                "including print_speed, travel_speed, z_hop, z_hop_speed, nozzle_diam, and the " +
                "ordered print/travel motion plan, plus nominal layer_w, flow-adjusted layer_wf, print_vol, " +
                "and any upstream assessment fields. Use this output for full printer simulation.",
                GH_ParamAccess.item);
        }
        #endregion
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            try
            {
                ...
                DA.GetDataList(0, sampleInfoRaw);

                // ---- 1 wsp_path ----------------------------------------------------
                wp = ReadGoo<WasperPrintPath, WasperPrintPathGoo>(DA, 1, "wsp_path");
                ...
                // ---- 2 3dp_params (unwired -> LDM defaults) -----------------------
                Wasper3dpParams prm =
                    ReadGoo<Wasper3dpParams, Wasper3dpParamsGoo>(DA, 2, "3dp_params")
                ...
                {
                    // Do not mutate the upstream Gc02/Gc03 object: Grasshopper may reuse it
                    // after the path input changes from a Gc08 path to a normal path.
                    prm = prm.Clone();
                ...
                if (isFdm)
                {
                    var gen = new FdmGcodeGenerator(
                        this, _versionTag,
                        points, flows, layerH,
                        prm, prm.Density, sampleInfoRaw, kpi, wp?.PrintVol);
            ...
            catch (Exception e)
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,
                    $"wsp_Gc04 unexpected error: {e.Message}");
                gTree = new DataTree<string>();
                gTree.Add($"; UNEXPECTED ERROR: {e.Message}", new GH_Path(0));
            }

            DA.SetDataTree(0, gTree);
            DA.SetDataTree(1, pointsOut);
            DA.SetDataTree(2, speedOut);
            DA.SetData(3, pTimeMin);
            DA.SetData(4, pPathLen);
            DA.SetData(5, pVolume);
            DA.SetData(6, layers);
            ...
            DA.SetData(7, new WasperPrintPathGoo(
                new WasperPrintPath(
        ...
        {
            private readonly wsp_Gc04_Marlin_Gcode_From_Points_Legacy _owner;
            private readonly string _versionTag;
            ...
            public LdmGcodeGenerator(
                wsp_Gc04_Marlin_Gcode_From_Points_Legacy owner, string versionTag,
            ...
            public bool Validate()
            {
                ...
                if (_printingPoints == null || _printingPoints.BranchCount == 0)
                    errors.Add("'p_points' is missing or empty in wsp_path. Supply a valid WASPer Print Path.");
                ...
                if (!_nozzleDiameterIn.HasValue || _nozzleDiameterIn.Value <= 0)
                    errors.Add("'nozzle_diameter' must be a positive number in mm — set it in the 3dp_params object (wsp_Gc02_LDM 3DP Parameters).");
                else
            ...
            private List<string> BuildPrintingInfoHeader()
            {
                return new List<string>
                {
                    ...
                    $";Plugin: WASPer_3DP - Generator: wsp_Gc04 LDM {_versionTag}",
            ...
            private void ProcessOneBranch(
                ...
                int branchIndex, int nPaths, List<GH_Path> paths)
            {
                int layerIndex = path.Indices[0];
                int curveIndex = path.Indices.Length > 1 ? path.Indices[1] : 0;
                ...
                bool isLastCurveInLayer =
                    ...
                    paths[branchIndex + 1].Indices[0] != layerIndex;

                if (ZHopV > 0 && isLastCurveInLayer)
                {
                ...
                double flux, double layerH, double speed, double packedVolume)
            {
                ...
                // Volume: effective cross-section × length (+15% empirical correction on the total)
        ...
        private sealed class FdmGcodeGenerator
        {
            ...
            public bool Validate()
            {
                ...
                if (_printingPoints == null || _printingPoints.BranchCount == 0)
                    errors.Add("Mandatory path field 'p_points' is missing or empty in wsp_path.");
                ...
                if (!_nozzleDiameterIn.HasValue || _nozzleDiameterIn.Value <= 0)
                    errors.Add("Mandatory input 'nozzle_diameter' must be a positive number in mm — set it in the 3dp_params object (wsp_Gc03_FDM 3DP Parameters).");
                else
                    _nozzleDiameter = _nozzleDiameterIn.Value;
            ...
            public void Run()
            {
                ...
                {
                    ";PRINTING INFORMATION:",
                    $";Generator: wsp_Gc04 FDM {_versionTag}",
                    $";Nozzle diameter: {Gc.F2(_nozzleDiameter)} [mm]",
                    $";Layer height (repr.): {Gc.F2(_layerHRepr)} [mm]",
